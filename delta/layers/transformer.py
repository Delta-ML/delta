...
class MultiHeadAttention(Layer):
  ...
  def call(self, inputs, training=None, mask=None):
    ...
    key = tf.matmul(
      key,
      tf.tile(tf.expand_dims(self.kernel_key, 0), [batch_size, 1, 1]))
    value = tf.matmul(
      value,
    ...
    if query_mask is not None:
      attention_outputs *= tf.cast(tf.expand_dims(query_mask, axis=-1), tf.float32)
...
class TransformerDecoderLayer(Layer):
  ...
  def __init__(self, config, **kwargs):
    ...
    # Self Attention
    self.self_attention_layer = MultiHeadSelfAttention(self.head_num,
                                                       self.attention_activation,
                                                       sequence_mask=True)
    self.self_attention_dropout_layer = tf.keras.layers.Dropout(self.dropout_rate)
    self.self_attention_ln = LayerNormalization()

    # Context Attention
    self.context_attention_layer = MultiHeadAttention(self.head_num,
                                                      self.attention_activation)
    self.context_attention_dropout_layer = tf.keras.layers.Dropout(self.dropout_rate)
  ...
  def call(self, inputs, training=None, mask=None):
    ...
    # Self
    self_attention = self.self_attention_layer(decoder_inputs, training=training, mask=decoder_mask)
    self_attention_dropout = self.self_attention_dropout_layer(
      self_attention, training=training)
    self_attention_out = tf.keras.layers.add([decoder_inputs, self_attention_dropout])
    self_attention_out = self.self_attention_ln(self_attention_out)

    # Context Attention
    context_attention = self.context_attention_layer([self_attention_out,
    ...
    context_attention_out = tf.keras.layers.add([self_attention_out, context_attention_dropout])
    ...
    feed_forward_dropout = self.feed_forward_dropout(
      feed_forward_layer, training=training)
    feed_forward_out = tf.keras.layers.add([context_attention_out, feed_forward_dropout])
...
class TransformerDecoder(Layer):
  ...
  def decode(self, input_dec_x, enc_out, enc_mask, training=None):
    ...
    for decoder_layer in self.transformer_decoders:
      dec_inp = decoder_layer([dec_inp, enc_out], training=training, mask=[dec_mask, enc_mask])
    dec_out = dec_inp
    return dec_out
  ...
  def call(self, inputs, training=None, mask=None):
    if not self.is_infer:
      ...
      enc_out = tf.tile(tf.expand_dims(enc_out, axis=1), [1, self.beam_size, 1, 1])
      enc_mask = tf.tile(tf.expand_dims(mask, axis=1), [1, self.beam_size, 1])
      enc_out = tf.reshape(enc_out, [enc_shape[0] * self.beam_size, enc_shape[1], enc_shape[2]])
      enc_mask = tf.reshape(enc_mask, [enc_shape[0] * self.beam_size, enc_shape[1]])

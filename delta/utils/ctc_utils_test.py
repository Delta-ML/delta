...
class CTCUtilTest(tf.test.TestCase):
  ...
  def test_transform_preprocess(self):
    ...
    with self.cached_session():
      ...
      with self.assertRaises(ValueError) as valueErr:
        labels = ctc_utils.transform_preprocess(labels=None, blank_index=None, num_class=None)
      ...
      with self.assertRaises(ValueError) as valueErr:
        labels = ctc_utils.transform_preprocess(labels=None, blank_index=10, num_class=10)
      the_exception = valueErr.exception
      self.assertEqual(
          str(the_exception),
          'blank_index must be less than or equal to num_class - 1')

      labels = ctc_utils.transform_preprocess(labels=None, blank_index=0, num_class=10)
      self.assertIsNone(labels)

      labels = ctc_utils.transform_preprocess(labels=tf.constant(self.labels),
                                              blank_index=0,
                                              num_class=10)
      labels_values = np.asarray([1, 1, 1, 3, 1, 1, 1])
      labels_index = np.asarray([[0, 0], [0, 1], [0, 2], [0, 3],[1, 0],[1, 1],[1, 2]])
  ...
  def test_labels_blankid_to_last(self):
    ...
    with self.cached_session():
      ...
      with self.assertRaises(AssertionError) as assert_err:
        labels = ctc_utils.labels_blankid_to_last(labels=self.labels, blank_index=0, num_class=None)
      the_exception = assert_err.exception
      self.assertEqual(
          str(the_exception),
          'The num_class should not be None!')
     
      labels = ctc_utils.labels_blankid_to_last(labels=tf.constant(self.labels),
                                                blank_index=0,
                                                num_class=6)
  ...
  def test_labels_last_to_blankid(self):
    ...
    with self.cached_session():
      ...
      labels = ctc_utils.labels_last_to_blankid(labels=tf.constant(self.labels),
                                                blank_index=0,
                                                num_class=None)

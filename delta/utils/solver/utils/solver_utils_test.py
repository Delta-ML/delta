...
class SolverUtilsTest(tf.test.TestCase):
  ...
  def test_get_model_file(self):
    ...
    file_name_list = ['best_model.ckpt', 'model.09-1.00.ckpt', 'model.00-1.00.ckpt']
    ...
    model_load_dict = {'best': Path(self.model_path).joinpath('best_model.ckpt'),
                       'specific': Path(self.model_path).joinpath(self.specified_model_file_name),
                       'latest': Path(self.model_path).joinpath('model.00-1.00.ckpt')}
    ...
    for cur_model_load_type in model_load_type_list:
      for cur_mode in mode_list:
        ...
        self.assertEqual(Path(self.model_path).joinpath('best_model.ckpt'), model_file_name)
    ...
    self.assertEqual(Path(self.model_path).joinpath('model.00-1.00.ckpt'), model_file_name)
  ...
  def test_get_most_recently_modified_file_matching_pattern(self):
    ...
    with open(Path(self.model_path).joinpath('model.01-1.00.h5'), 'w', encoding='utf-8') as f:
      f.write('test ckpt file')
    most_rencently_modified_file = solver_utils.get_most_recently_modified_file_matching_pattern(
        self.model_path,
        self.file_name_pattern)
    ...
    file_name_list = ['model.01-1.00.ckpt', 'model.02-1.00.ckpt', 'model.03-1.00.ckpt']
    for file_name in file_name_list:
      file_path = Path(self.model_path).joinpath(file_name)
      with open(file_path, 'w', encoding='utf-8') as f: #pylint: disable=invalid-name
        f.write('test ckpt file')

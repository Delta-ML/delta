...
class TextMatchTask(TextTask):
  ...
  def feature_spec(self):
    ...
    feature_shapes = [
        tuple(feature_shapes), (tf.TensorShape([]), tf.TensorShape([]))
    ]
  ...
  def dataset(self):
    ...
    if self.infer_without_label:
      (input_x_left, input_x_right), (input_x_left_len,
                                      input_x_right_len) = iterator.get_next()
    else:
      ((input_x_left, input_x_right),
       input_y), (input_x_left_len, input_x_right_len) = iterator.get_next()

    input_x_dict = collections.OrderedDict([
        ("input_x_left", input_x_left),

...
PYBIND11_MODULE(export_py, module) {
    ...
    py::class_<grappler::Pattern>(core, "Pattern")
        ...
        .def("LoadModel", delta_overload_cast<const char*>()(&grappler::Pattern::LoadModel), "Load tf model from pb files.")
        .def("LoadModelCT", delta_overload_cast<const std::string&>()(&grappler::Pattern::LoadModelCT), "Load tf model from GraphDef string buffer.")
        ...
        .def("node", delta_overload_cast<int>()(&grappler::Pattern::node, py::const_), "Get Node by index.")
        .def("node", delta_overload_cast<const std::string&>()(&grappler::Pattern::node, py::const_), "Get Node by name.")
    ...
    py::class_<grappler::LocalOptimizer, PyLocalOptimizer> RealPyLocalOptimizer(optimizer, "LocalOptimizer");
    RealPyLocalOptimizer.def(py::init<const std::string&>())
                    ...
                    .def("_repr__", [](const grappler::LocalOptimizer& optim) {
                        return  "<[Pure Abstract c++ class]export_py.LocalOptimizer named '" + optim.GetName()+ "'>";
                    });

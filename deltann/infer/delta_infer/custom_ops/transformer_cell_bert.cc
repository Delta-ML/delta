...
public:
    explicit TansformerCellBertOp(OpKernelConstruction* context) : OpKernel(context) {
    ...
    void Compute(OpKernelContext* context) override {
        ...
        // mask
        _param.mask = reinterpret_cast<const int *>(context->input(12).flat<float>().data()); // matter
        ...
        _param.output_kernel = reinterpret_cast<const typename traits::DataType *>(context->input(34).flat<T>().data());
        _param.output_bias = reinterpret_cast<const typename traits::DataType *>(context->input(30).flat<T>().data());
        _param.output_layernorm_beta = reinterpret_cast<const typename traits::DataType *>(context->input(38).flat<T>().data());
        _param.output_layernorm_gamma = reinterpret_cast<const typename traits::DataType *>(context->input(36).flat<T>().data()); 
...
#define REGISTER_CPU(T)                                          \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("TransformerCellBertOther").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
...
#define REGISTER_GPU(T)                                          \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("TransformerCellBertOther").Device(DEVICE_GPU).TypeConstraint<T>("T"), \

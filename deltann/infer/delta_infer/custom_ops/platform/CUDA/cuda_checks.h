...
#define ERROR_STATUS(Value) case Value: return #Value
...
#define cuda(function) do {                                                                 \
    cudaError_t status =  (cuda##function);                                                 \
    if ( cudaSuccess != (status) ) {                                                        \
        fprintf( stderr, "[ERROR] CUDA Runtime Failure (line %d of file %s):\n\t"           \
                         " %s returned 0x%x (%s)\n",                                        \
...
#define cublas(function) do {                                                               \
    cublasStatus_t status =  (cublas##function);                                            \
    if ( CUBLAS_STATUS_SUCCESS != (status) ) {                                              \
        fprintf( stderr, "[ERROR] Cublas Runtime Failure (line %d of file %s):\n\t"         \
                         " %s returned 0x%x (%s)\n",                                        \
                         __LINE__, __FILE__, #function, status, cubaseGetErrStr(status) );  \
        exit(1);                                                                            \
    }                                                                                       \
...
#define cudnn(function) do {                                                                \
    cublasStatus_t status =  (cudnn##function);                                             \
    if ( CUDNN_STATUS_SUCCESS != (status) ) {                                               \
        fprintf( stderr, "[ERROR] Cudnn Runtime Failure (line %d of file %s):\n\t"          \
                         " %s returned 0x%x (%s)\n",                                        \
                         __LINE__, __FILE__, #function, status, cubaseGetErrStr(status) );  \
        exit(1);                                                                            \
    }                                                                                       \

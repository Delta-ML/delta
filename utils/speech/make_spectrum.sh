...
if [ $# -ge 3 ]; then
  ...
  spectrum_dir=$data/data
fi

# make $spectrum_dir an absolute pathname.
spectrum_dir=`perl -e '($dir,$pwd)= @ARGV; if($dir!~m:^/:) { $dir = "$pwd/$dir"; } print $dir; ' $spectrum_dir ${PWD}`

# use "name" as part of name of the archive.
name=`basename $data`

mkdir -p $spectrum_dir || exit 1;
...
if [ -f ${data}/segments ]; then
    ...
    ${cmd} JOB=1:${nj} ${logdir}/make_spectrum${name}.JOB.log \
        speech/compute_spectrum_feats.py \
            ...
            ark,scp:${spectrum_dir}/raw_spectrum${name}.JOB.${ext},${spectrum_dir}/raw_spectrum${name}.JOB.scp
...
for n in $(seq ${nj}); do
    cat ${spectrum_dir}/raw_spectrum${name}.${n}.scp || exit 1;
...
echo "Succeeded creating spectrum features for $name"
